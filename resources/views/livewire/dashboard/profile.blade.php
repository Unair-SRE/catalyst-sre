<div>
    <x-ui.container class="py-8 sm:py-10 lg:py-12">
        <div class="max-w-3xl space-y-8 lg:space-y-10">
            <header>
                <p class="text-sm text-catalyst-grey">Account settings</p>
                <h1 class="mt-5 font-display text-3xl font-semibold tracking-tight text-catalyst-black sm:text-4xl">Profile</h1>
                <p class="mt-4 text-base leading-7 text-catalyst-ink/80 sm:text-lg">Manage your personal information and account security.</p>
            </header>

            <form class="border border-catalyst-grey/30 bg-white p-5 shadow-sm sm:p-6" wire:submit="saveProfile" novalidate>
                <div>
                ...
                <div class="mt-6 space-y-5">
                    <div>
                        ...
                        <p id="profile-name-help" class="mt-2 text-xs leading-5 text-catalyst-grey">Use your legal name. This name may be used for certificates and official Catalyst documents.</p>
                        @error('profile.name') <p id="profile-name-error" class="mt-1 text-sm text-status-error">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <p id="profile-email-help" class="mt-2 text-xs leading-5 text-catalyst-grey">{{ $state['email_notice'] }}</p>
                        @error('profile.email') <p id="profile-email-error" class="mt-1 text-sm text-status-error">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('profile.whatsapp') <p id="profile-whatsapp-error" class="mt-2 text-sm text-status-error">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('profile.institution') <p id="profile-institution-error" class="mt-2 text-sm text-status-error">{{ $message }}</p> @enderror
                    </div>
                </div>
                ...
                @if ($profileFeedback)
                    <p class="mt-5 text-sm font-medium text-status-success" role="status">{{ $profileFeedback }}</p>
                @endif
            ...
            <form class="border border-catalyst-grey/30 bg-white p-5 shadow-sm sm:p-6" wire:submit="updatePassword" novalidate>
                <div>
                ...
                <div class="mt-6 space-y-5">
                    <div>
                        ...
                        @error('password.current') <p id="current-password-error" class="mt-2 text-sm text-status-error">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        <p id="new-password-help" class="mt-2 text-xs leading-5 text-catalyst-grey">Use at least {{ $state['password_policy']['minimum_length'] }} characters for this prototype.</p>
                        @error('password.new') <p id="new-password-error" class="mt-1 text-sm text-status-error">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        @error('password.confirmation') <p id="password-confirmation-error" class="mt-2 text-sm text-status-error">{{ $message }}</p> @enderror
                    </div>
                </div>

                @if ($passwordFeedback)
                    <p class="mt-5 text-sm font-medium text-status-success" role="status">{{ $passwordFeedback }}</p>
                @endif
