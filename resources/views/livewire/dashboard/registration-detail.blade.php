<div>
    <x-ui.container class="py-8 sm:py-10 lg:py-14">
        <div class="space-y-8 lg:space-y-10">
            <header class="dashboard-page-heading max-w-3xl">
                ...
                <p class="mt-6 text-sm font-medium tracking-widest text-catalyst-muted">{{ $state['competition']['short_name'] }}</p>
                <h1 class="mt-2 font-display text-3xl font-medium leading-tight tracking-tight text-catalyst-ink sm:text-4xl xl:text-5xl">{{ $state['competition']['name'] }}</h1>
            ...
            @if ($state['notice'])
                <section class="border-l-4 bg-white p-5" style="border-left-color: var(--color-status-{{ $state['notice']['tone'] }})" aria-live="polite">
            ...
            <section aria-labelledby="flow-heading" class="border border-catalyst-grey/30 bg-white p-5">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                    <h2 id="flow-heading" class="font-display text-xl font-semibold tracking-tight">Registration Flow</h2>
                    <p class="text-xs font-medium tracking-wide text-catalyst-muted">Review your details before submitting.</p>
                </div>
                <ol class="mt-5 flex gap-3 overflow-x-auto pb-3" aria-label="Registration progress">
                    @foreach (['Choose Competition', 'Confirm Eligibility', 'Team Data', 'Member Data', 'Registration Folder', 'Payment', 'Review'] as $step)
                        <li class="flex min-w-28 flex-1 items-start gap-2 text-xs leading-5 text-catalyst-ink/75 sm:min-w-0">
                            <span class="grid size-6 shrink-0 place-items-center border border-catalyst-primary/25 bg-catalyst-surface text-xs font-semibold text-catalyst-primary">{{ $loop->iteration }}</span>
                            <span>{{ $step }}</span>
                        </li>
                    @endforeach
                </ol>
            ...
            <form class="space-y-10" wire:submit="openConfirmation">
                <fieldset class="min-w-0 border-t border-catalyst-grey/30 bg-white pt-5 pb-3 sm:pt-6" @disabled(! $state['form_editable'])>
                    ...
                    @error('form.eligible') <p class="mt-2 text-sm text-status-error-ink">{{ $message }}</p> @enderror
                </fieldset>

                <fieldset class="min-w-0 border-t border-catalyst-grey/30 bg-white pt-5 pb-3 sm:pt-6" @disabled(! $state['form_editable'])>
                    <legend class="px-1 font-display text-xl font-semibold tracking-tight">Team Data</legend>
                    <div class="mt-5 grid gap-5 sm:grid-cols-2">
                        <label class="block text-sm font-medium">Team Name<input class="mt-2 w-full border border-catalyst-grey/50 bg-white px-3 py-3 text-sm focus:border-catalyst-primary focus:outline-none disabled:bg-catalyst-grey/10" type="text" wire:model="form.team_name">@error('form.team_name') <span class="mt-1 block text-sm text-status-error-ink">{{ $message }}</span> @enderror</label>
                        <label class="block text-sm font-medium">Institution / School / University<input class="mt-2 w-full border border-catalyst-grey/50 bg-white px-3 py-3 text-sm focus:border-catalyst-primary focus:outline-none disabled:bg-catalyst-grey/10" type="text" wire:model="form.institution">@error('form.institution') <span class="mt-1 block text-sm text-status-error-ink">{{ $message }}</span> @enderror</label>
                    </div>
                </fieldset>
                ...
                <fieldset class="min-w-0 border-t border-catalyst-grey/30 bg-white pt-5 pb-3 sm:pt-6" @disabled(! $state['form_editable'])>
                    ...
                    <div class="mt-5 space-y-5">
                        @foreach ($form['members'] as $index => $member)
                            <section class="bg-catalyst-neutral p-4 sm:p-5" aria-labelledby="member-{{ $index }}-heading">
                                ...
                                <div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                    ...
                                    <label class="block text-sm">Student ID <span class="text-catalyst-muted">(optional)</span><input class="mt-2 w-full border border-catalyst-grey/50 bg-white px-3 py-3 text-sm focus:border-catalyst-primary focus:outline-none disabled:bg-catalyst-grey/10" type="text" wire:model="form.members.{{ $index }}.student_id"></label>
                                </div>
                            </section>
                        @endforeach
                    </div>
                ...
                <section class="border border-catalyst-grey/30 bg-white p-5 sm:p-6" aria-labelledby="review-heading">
                    <h2 id="review-heading" class="font-display text-xl font-semibold tracking-tight">Review</h2>
                    <dl class="mt-5 grid gap-4 text-sm sm:grid-cols-2">
                        <div><dt class="text-catalyst-muted">Competition</dt><dd class="mt-1 font-medium">{{ $state['competition']['name'] }}</dd></div>
                        <div><dt class="text-catalyst-muted">Team</dt><dd class="mt-1 font-medium">{{ $form['team_name'] ?: 'Not set' }}</dd></div>
                        <div><dt class="text-catalyst-muted">Institution</dt><dd class="mt-1 font-medium">{{ $form['institution'] ?: 'Not set' }}</dd></div>
                        <div><dt class="text-catalyst-muted">Members</dt><dd class="mt-1 font-medium">{{ collect($form['members'])->filter(fn ($member) => filled($member['legal_name']))->count() }} people</dd></div>
                        <div><dt class="text-catalyst-muted">Registration folder</dt><dd class="mt-1 font-medium">{{ $form['drive_url'] ? 'Provided' : 'Missing' }}</dd></div>
                        <div><dt class="text-catalyst-muted">Payment</dt><dd class="mt-1"><x-dashboard.status-pill :label="$state['payment']['label']" :tone="$state['payment']['tone']" /></dd></div>
                    </dl>
                </section>

                <section class="border border-catalyst-grey/30 bg-white p-5 sm:p-6" aria-labelledby="whatsapp-heading">
                    ...
                    @else
                        <p class="mt-4 text-sm text-catalyst-muted">Available immediately after you submit your registration.</p>
                    @endif
                </section>

                @if ($state['form_editable'])
                    <button class="w-full sm:w-auto bg-catalyst-primary px-4 py-4 text-sm font-medium text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-catalyst-primary" type="submit">Submit Registration</button>
                @endif
            </form>
    ...
    @if ($showConfirm)
        <dialog x-data x-init="$el.showModal()" @cancel.prevent="$wire.cancelConfirmation()" class="fixed inset-0 m-auto max-h-[90dvh] w-[calc(100%_-_2.5rem)] max-w-md overflow-y-auto border-0 bg-white p-0 text-catalyst-ink backdrop:bg-catalyst-ink/40" aria-labelledby="submit-registration-title">
