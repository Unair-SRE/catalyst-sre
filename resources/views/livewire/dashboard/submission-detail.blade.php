...
>
    <x-ui.container class="py-8 sm:py-10 lg:py-12">
        <div class="space-y-8 lg:space-y-10">
            <header class="max-w-4xl">
                ...
                <p class="mt-8 text-sm font-medium tracking-wide text-catalyst-grey">{{ $state['competition']['short_name'] }} · {{ $state['stage']['name'] }}</p>
                <h1 class="mt-3 font-display text-3xl font-semibold tracking-tight text-catalyst-black sm:text-4xl">{{ $state['competition']['name'] }}</h1>
            ...
            @elseif ($state['stage_state'] === 'upcoming')
                <section class="border border-catalyst-grey/30 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="submission-upcoming-heading">
            ...
            @elseif ($state['stage_state'] === 'closed')
                <section class="border border-catalyst-grey/30 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="submission-closed-heading">
            ...
            @elseif (in_array($state['submission_state'], ['submitted', 'late'], true))
                <section class="border border-status-success/30 bg-status-success/5 p-5 sm:p-6" aria-labelledby="submission-received-heading">
            ...
            <section class="border border-catalyst-grey/30 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="submission-requirements-heading">
                ...
                <dl class="mt-6 grid gap-5 text-sm sm:grid-cols-2 lg:grid-cols-4">
                    <div><dt class="text-catalyst-grey">Formats</dt><dd class="mt-1 font-medium">{{ $state['requirements']['allowed_label'] }}</dd></div>
                    <div><dt class="text-catalyst-grey">File size</dt><dd class="mt-1 font-medium">{{ $state['requirements']['max_file_label'] }}</dd></div>
                    <div><dt class="text-catalyst-grey">Maximum files</dt><dd class="mt-1 font-medium">{{ $state['requirements']['max_files'] }} files</dd></div>
                    <div><dt class="text-catalyst-grey">Deadline</dt><dd class="mt-1 font-medium"><time datetime="{{ $state['stage']['deadline_at'] }}">{{ $state['stage']['deadline_label'] }}</time></dd></div>
                </dl>
            ...
            @if ($can_edit)
                <section class="border border-catalyst-grey/30 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="upload-heading">
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                        ...
                        <p class="text-sm text-catalyst-grey">{{ count($state['files']) }} / {{ $state['requirements']['max_files'] }} files</p>
                    </div>
                    ...
                    <div class="mt-5 grid min-h-44 place-items-center border border-dashed border-catalyst-grey/60 bg-catalyst-grey/5 p-6 text-center" @drop.prevent="selectFiles($event.dataTransfer.files)" @dragover.prevent>
                        <div>
                    ...
                    <template x-if="clientErrors.length">
                        <div class="mt-4 space-y-2" aria-live="polite">
                            <template x-for="error in clientErrors" :key="error"><p class="border border-status-error/30 bg-status-error/5 p-3 text-sm leading-6 text-catalyst-ink"><span class="font-medium text-status-error">Upload issue.</span> <span x-text="error"></span></p></template>
                        </div>
                    </template>
                    ...
                    @if ($uploadErrors !== [])
                        <div class="mt-4 space-y-2" aria-live="polite">
                            @foreach ($uploadErrors as $error)
                                <p class="border border-status-error/30 bg-status-error/5 p-3 text-sm leading-6 text-catalyst-ink"><span class="font-medium text-status-error">Upload issue.</span> {{ $error }}</p>
                            @endforeach
                        </div>
                    @endif
                </section>
            ...
            @if ($state['files'] !== [])
                <section class="border border-catalyst-grey/30 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="selected-files-heading">
                        ...
                        @foreach ($state['files'] as $file)
                            <li class="py-4 first:pt-0 last:pb-0" wire:key="submission-file-{{ $file['id'] }}">
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-medium text-catalyst-ink">{{ $file['name'] }}</p>
                                        <p class="mt-1 text-xs text-catalyst-grey">{{ $file['size_label'] }} · {{ strtoupper(pathinfo($file['name'], PATHINFO_EXTENSION)) }}</p>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        @if ($file['state'] === 'uploaded')
                                            <span class="text-sm font-medium text-status-success">Uploaded</span>
                                        @elseif ($file['state'] === 'uploading')
                                            <span class="text-sm font-medium text-catalyst-ink/75">Uploading {{ $file['progress'] }}%</span>
                                        @else
                                            <span class="text-sm font-medium text-status-error">Upload interrupted</span>
                                        @endif
                                        @if ($can_edit)
            ...
            @if ($can_edit)
                <button class="inline-flex min-h-12 w-full items-center justify-center bg-linear-to-b from-catalyst-primary to-catalyst-green px-4 py-3 text-sm font-medium text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-catalyst-primary" type="button" wire:click="openSubmitConfirmation">{{ $state['submission_state'] === 'revision_required' ? 'Submit Updated Entry' : 'Submit Entry' }}</button>
            @endif

            @if ($state['history'] !== [])
                <section class="border border-catalyst-grey/30 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="submission-history-heading">
    ...
    @if ($showSubmitConfirm)
        <div class="fixed inset-0 z-30 grid place-items-center bg-catalyst-ink/40 p-5" role="dialog" aria-modal="true" aria-labelledby="submit-entry-title" wire:keydown.escape="cancelSubmitConfirmation">
    ...
    @if ($showUnsubmitConfirm)
        <div class="fixed inset-0 z-30 grid place-items-center bg-catalyst-ink/40 p-5" role="dialog" aria-modal="true" aria-labelledby="unsubmit-entry-title" wire:keydown.escape="cancelUnsubmitConfirmation">
            <div class="w-full max-w-md border border-catalyst-grey/30 bg-white p-6 shadow-lg">
