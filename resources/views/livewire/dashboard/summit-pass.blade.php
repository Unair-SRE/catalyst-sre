<div>
    <x-ui.container class="py-8 sm:py-10 lg:py-14">
        <div class="space-y-8 lg:space-y-10">
            <header class="dashboard-page-heading max-w-3xl">
                <p class="text-xs font-medium tracking-wide text-catalyst-muted">Summit experience</p>
                <h1 class="mt-5 font-display text-3xl font-medium leading-tight tracking-tight text-catalyst-ink sm:text-4xl xl:text-5xl">Summit Pass</h1>
            ...
            @if ($state['status'] === 'NO_PASS')
                <section class="max-w-3xl border border-catalyst-grey/30 bg-white p-6 sm:p-8" aria-labelledby="summit-empty-heading">
                    ...
                    <dl class="mt-7 grid gap-5 border-catalyst-grey/30 border-y py-5 text-sm sm:grid-cols-3">
                        <div><dt class="text-catalyst-muted">Date</dt><dd class="mt-1 font-medium">{{ $state['event']['date_label'] }} · {{ $state['event']['time_label'] }}</dd></div>
                        <div><dt class="text-catalyst-muted">Venue</dt><dd class="mt-1 font-medium">{{ $state['event']['venue'] }}</dd></div>
                        <div><dt class="text-catalyst-muted">Ticket price</dt><dd class="mt-1 font-medium">{{ $state['event']['price'] }}</dd></div>
                    </dl>
            ...
            @if ($form_editable)
                <form class="max-w-4xl space-y-6" wire:submit="openConfirmation">
                    ...
                    <fieldset class="border border-catalyst-grey/30 bg-white p-5 sm:p-6">
                        ...
                        <div class="mt-5 grid gap-5 sm:grid-cols-2">
                            <div>
                                ...
                                @error('attendee.name') <p id="summit-name-error" class="mt-2 text-sm text-status-error-ink">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                ...
                                @error('attendee.email') <p id="summit-email-error" class="mt-2 text-sm text-status-error-ink">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                ...
                                @error('attendee.whatsapp') <p id="summit-whatsapp-error" class="mt-2 text-sm text-status-error-ink">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                ...
                                @error('attendee.institution') <p id="summit-institution-error" class="mt-2 text-sm text-status-error-ink">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </fieldset>

                    <section class="border border-catalyst-grey/30 bg-white p-5 sm:p-6" aria-labelledby="pass-information-heading">
                        ...
                        <dl class="mt-5 grid gap-5 text-sm sm:grid-cols-2">
                            <div><dt class="text-catalyst-muted">Access</dt><dd class="mt-1 font-medium">{{ $state['event']['access'] }}</dd></div>
                            <div><dt class="text-catalyst-muted">Event date</dt><dd class="mt-1 font-medium">{{ $state['event']['date_label'] }} · {{ $state['event']['time_label'] }}</dd></div>
                            <div><dt class="text-catalyst-muted">Venue</dt><dd class="mt-1 font-medium">{{ $state['event']['venue'] }}</dd></div>
                            <div><dt class="text-catalyst-muted">Ticket price</dt><dd class="mt-1 font-medium">{{ $state['event']['price'] }}</dd></div>
                        </dl>
                            ...
                            <div>
                                ...
                                <div class="mt-5 grid gap-4 sm:grid-cols-2">
                                    <div class="sm:col-span-2">
                                        ...
                                        @error('payment.sender_name') <p id="summit-sender-error" class="mt-2 text-sm text-status-error-ink">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        ...
                                        @error('payment.date') <p id="summit-payment-date-error" class="mt-2 text-sm text-status-error-ink">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        ...
                                        @error('payment.time') <p id="summit-payment-time-error" class="mt-2 text-sm text-status-error-ink">{{ $message }}</p> @enderror
                                    </div>
                                </div>
                    ...
                    <section class="border border-catalyst-grey/30 bg-white p-5 sm:p-6" aria-labelledby="summit-review-heading">
                        <h2 id="summit-review-heading" class="font-display text-xl font-semibold tracking-tight">Review</h2>
                        <dl class="mt-5 grid gap-4 text-sm sm:grid-cols-2">
                            <div><dt class="text-catalyst-muted">Attendee</dt><dd class="mt-1 font-medium">{{ $state['attendee']['name'] ?: 'Not set' }}</dd></div>
                            <div><dt class="text-catalyst-muted">Access</dt><dd class="mt-1 font-medium">{{ $state['event']['access'] }}</dd></div>
                            <div><dt class="text-catalyst-muted">Ticket price</dt><dd class="mt-1 font-medium">{{ $state['event']['price'] }}</dd></div>
                            <div><dt class="text-catalyst-muted">Payment proof</dt><dd class="mt-1 font-medium">{{ $state['payment']['proof'] ? 'Selected' : 'Missing' }}</dd></div>
                        </dl>
                    </section>

                    <button class="inline-flex min-h-12 w-full sm:w-auto items-center justify-center bg-catalyst-primary px-4 py-3 text-sm font-medium text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-catalyst-primary" type="submit">{{ $state['status'] === 'REJECTED' ? 'Resubmit Purchase' : 'Submit Purchase' }}</button>
                </form>
            @endif
            ...
            @if ($state['status'] === 'WAITING_VERIFICATION')
                <section class="max-w-4xl border border-status-info/30 bg-status-info/5 p-6 sm:p-8" aria-labelledby="payment-waiting-heading">
                    ...
                    <dl class="mt-6 grid gap-5 border-catalyst-grey/30 border-t pt-5 text-sm sm:grid-cols-2 lg:grid-cols-4">
                        <div><dt class="text-catalyst-muted">Attendee</dt><dd class="mt-1 font-medium">{{ $state['attendee']['name'] }}</dd></div>
                        <div><dt class="text-catalyst-muted">Ticket price</dt><dd class="mt-1 font-medium">{{ $state['event']['price'] }}</dd></div>
                        <div><dt class="text-catalyst-muted">Sender</dt><dd class="mt-1 font-medium">{{ $state['payment']['sender_name'] }}</dd></div>
                        <div><dt class="text-catalyst-muted">Submitted</dt><dd class="mt-1 font-medium">{{ $state['submitted_label'] }}</dd></div>
                    </dl>
            ...
            @if (in_array($state['status'], ['VERIFIED', 'CHECKED_IN'], true))
                <section class="max-w-4xl" aria-labelledby="active-ticket-heading">
                    ...
                    <article class="summit-ticket overflow-hidden border border-catalyst-primary/20 bg-catalyst-surface p-6 text-catalyst-ink sm:p-8">
                        <div class="grid gap-8 md:grid-cols-[minmax(0,1fr)_13rem] md:items-center">
                            <div>
                                <p class="text-sm text-catalyst-ink/75">Catalyst 2026</p>
                                <h3 class="mt-3 font-display text-3xl font-semibold tracking-tight">{{ $state['event']['name'] }}</h3>
                                <p class="mt-2 text-base text-catalyst-ink/85">{{ $state['event']['access'] }}</p>
                                <dl class="mt-7 grid gap-4 text-sm sm:grid-cols-2">
                                    <div><dt class="text-catalyst-muted">Attendee</dt><dd class="mt-1 font-medium">{{ $state['attendee']['name'] }}</dd></div>
                                    <div><dt class="text-catalyst-muted">Ticket ID</dt><dd class="mt-1 font-medium tracking-wide">{{ $state['ticket']['id'] }}</dd></div>
                                    <div><dt class="text-catalyst-muted">Date</dt><dd class="mt-1 font-medium">{{ $state['event']['date_label'] }} · {{ $state['event']['time_label'] }}</dd></div>
                                    <div><dt class="text-catalyst-muted">Venue</dt><dd class="mt-1 font-medium">{{ $state['event']['venue'] }}</dd></div>
                                </dl>
                                <p class="mt-7 max-w-xl text-sm leading-6 text-catalyst-ink/75">This pass is assigned to your account and is non-transferable. Do not share the future production ticket code.</p>
                            </div>
                            <div class="grid aspect-square w-full max-w-52 place-items-center justify-self-center border border-dashed border-catalyst-primary/30 bg-white p-5 text-center text-catalyst-ink md:justify-self-end" role="img" aria-label="{{ $state['ticket']['qr_label'] }}">
                                <div><svg class="mx-auto size-8" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M4 4h6v6H4zm10 0h6v6h-6zM4 14h6v6H4zm11 0h2v2h-2zm3 0h2v5h-2zm-3 4h2v2h-2z" stroke="currentColor" stroke-width="1.5" /></svg><p class="mt-3 text-sm font-medium">Prototype QR</p><p class="mt-1 text-xs leading-5 text-catalyst-muted">Ticket QR Placeholder</p></div>
                            </div>
                        </div>
                    </article>
                </section>
    ...
    @if ($showConfirm)
        <dialog x-data x-init="$el.showModal()" @cancel.prevent="$wire.cancelConfirmation()" class="fixed inset-0 m-auto max-h-[90dvh] w-[calc(100%_-_2.5rem)] max-w-md overflow-y-auto border-0 bg-white p-0 text-catalyst-ink backdrop:bg-catalyst-ink/40" aria-labelledby="submit-summit-purchase-title">
